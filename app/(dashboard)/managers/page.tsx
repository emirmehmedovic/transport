...
export default function ManagersPage() {
  ...
  const fetchManagers = async (overrides?: {
    search?: string;
    statusFilter?: "all" | "ACTIVE" | "VACATION" | "INACTIVE";
    page?: number;
    pageSize?: number;
    ...
    try {
      ...
      const url = query ? `/api/managers?${query}` : "/api/managers";
  ...
  const totalManagers = pagination.total || managers.length;
  const managerStats = managers.reduce(
    (acc, manager) => {
      ...
      if (manager.department) {
        acc.withDepartment += 1;
      }
      return acc;
    },
    { active: 0, inactive: 0, withDepartment: 0 }
  );
  ...
  return (
    <div className="space-y-4 md:space-y-6 lg:space-y-8 font-sans px-4 md:px-0">
      <PageHeader
        icon={Briefcase}
        title="Menadžeri"
        subtitle="Upravljajte vašim menadžerima i pratite njihov status."
        actions={
          <button
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
          <div className="bg-white/5 rounded-xl md:rounded-2xl px-4 md:px-5 py-2.5 md:py-3 border border-white/10">
            ...
            <p className="text-xl md:text-2xl font-bold mt-1">{totalManagers}</p>
          </div>
          <div className="bg-white/5 rounded-xl md:rounded-2xl px-4 md:px-5 py-2.5 md:py-3 border border-white/10">
            <p className="text-[10px] md:text-xs font-semibold text-dark-200 uppercase tracking-wide">
              Aktivni
            </p>
            <p className="text-xl md:text-2xl font-bold mt-1">{managerStats.active}</p>
          </div>
          <div className="bg-white/5 rounded-xl md:rounded-2xl px-4 md:px-5 py-2.5 md:py-3 border border-white/10">
            <p className="text-[10px] md:text-xs font-semibold text-dark-200 uppercase tracking-wide">
              Sa odjelom
            </p>
            <p className="text-xl md:text-2xl font-bold mt-1">{managerStats.withDepartment}</p>
          </div>
        </div>
      </PageHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
        <div className="relative md:col-span-2 group">
          ...
          <div className="relative flex items-center px-4 md:px-6 py-3 md:py-4">
            <Search className="w-4 h-4 md:w-5 md:h-5 text-dark-400 group-focus-within:text-orange-500 transition-colors flex-shrink-0" />
            <input
              type="text"
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  ...
                  setLoading(true);
                  setPage(1);
                  fetchManagers({ search: e.currentTarget.value, page: 1 });
                }
              }}
            />
          </div>
          ...
          <div className="relative flex items-center px-4 md:px-6 py-3 md:py-4">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setPage(1);
                setLoading(true);
                fetchManagers({ statusFilter: value, page: 1 });
              }}
            >
          ...
          <div className="flex items-center gap-1.5 md:gap-2 bg-white rounded-full px-3 md:px-4 py-1.5 md:py-2 shadow-soft">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setPage(1);
                setLoading(true);
                fetchManagers({ sortBy: nextSortBy, sortDir: nextSortDir, page: 1 });
              }}
            >
          ...
          <div className="flex items-center gap-1.5 md:gap-2 bg-white rounded-full px-3 md:px-4 py-1.5 md:py-2 shadow-soft">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setPage(1);
                setLoading(true);
                fetchManagers({ pageSize: value, page: 1 });
              }}
            >
      ...
      <div className="bg-white/95 backdrop-blur-sm rounded-3xl shadow-lg border border-slate-200/60 overflow-hidden min-h-[300px] md:min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center px-4">
            <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center text-orange-400 mb-6 ring-4 ring-orange-100">
              <Briefcase className="w-10 h-10" />
            </div>
            <h3 className="text-xl font-bold text-slate-900 mb-2">Nema pronađenih menadžera</h3>
            <p className="text-slate-600 max-w-xs mx-auto">
              Pokušajte promijeniti filtere ili dodajte novog menadžera u sistem.
            </p>
            <button
              onClick={() => router.push("/managers/new")}
              className="mt-6 px-6 py-3 rounded-xl bg-orange-600 text-white font-semibold hover:bg-orange-700 transition-all shadow-lg"
            >
              + Dodaj novog menadžera
            </button>
          </div>
        ) : (
          <div>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 ring-2 ring-orange-200">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-dark-600">
                    <span className="rounded-full bg-dark-50 px-2 py-0.5">
                      Telefon: {manager.user.phone || "-"}
                    </span>
                    <span className="rounded-full bg-dark-50 px-2 py-0.5">
                      Odjel: {manager.department || "Nije određeno"}
                    </span>
                  </div>
                  <div className="mt-3 flex items-center justify-end gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/managers/${manager.id}/edit`);
                      }}
                      className="px-3 py-1.5 text-xs font-semibold rounded-lg border border-dark-100 text-dark-600 hover:text-orange-600 hover:border-orange-200"
                    >
                      Uredi
                    </button>
                    <button
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-xs md:text-sm text-left">
              <thead>
                <tr className="bg-gradient-to-r from-orange-50 to-orange-100/50 border-b border-orange-200">
                  <th className="px-3 md:px-6 py-3 md:py-4 text-xs font-semibold text-orange-700 uppercase tracking-wider whitespace-nowrap">Menadžer</th>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-xs font-semibold text-orange-700 uppercase tracking-wider whitespace-nowrap hidden lg:table-cell">Kontakt</th>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-xs font-semibold text-orange-700 uppercase tracking-wider whitespace-nowrap hidden xl:table-cell">Odjel</th>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-xs font-semibold text-orange-700 uppercase tracking-wider whitespace-nowrap hidden md:table-cell">Datum zaposlenja</th>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-xs font-semibold text-orange-700 uppercase tracking-wider whitespace-nowrap">Status</th>
                  <th className="px-3 md:px-6 py-3 md:py-4 text-xs font-semibold text-orange-700 uppercase tracking-wider text-right whitespace-nowrap">Akcije</th>
                </tr>
              </thead>
                  ...
                  >
                    <td className="px-3 md:px-6 py-3 md:py-4">
                      <div className="flex items-center gap-2 md:gap-3">
                        <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center text-white font-bold shadow-md text-xs md:text-sm flex-shrink-0 ring-2 ring-orange-200">
                          {manager.user.firstName[0]}
                          {manager.user.lastName[0]}
                        </div>
                        <div className="min-w-0">
                    ...
                    <td className="px-3 md:px-6 py-3 md:py-4 hidden lg:table-cell">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2 text-slate-700">
                          ...
                          <span className="truncate font-medium">{manager.user.email}</span>
                        </div>
                        <div className="flex items-center gap-2 text-slate-600">
                          <Phone className="w-3 h-3 flex-shrink-0 text-slate-400" />
                          <span className="truncate">{manager.user.phone || "Nema telefon"}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-3 md:px-6 py-3 md:py-4 hidden xl:table-cell">
                      <div className="flex items-center gap-2">
                        <div className="p-1.5 md:p-2 bg-orange-100 rounded-xl text-orange-600 flex-shrink-0">
                    ...
                    <td className="px-3 md:px-6 py-3 md:py-4 text-right">
                      <div className="flex items-center justify-end gap-1 md:gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="p-1.5 md:p-2 text-slate-500 hover:text-orange-600 hover:bg-orange-50 rounded-lg md:rounded-xl transition-all"
                          title="Uredi"
                        >
